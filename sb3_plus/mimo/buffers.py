...
class MultiOutputReplayBuffer(MultiOutputBaseBuffer):
    ...
    def __init__(
        ...
        self.actions = np.zeros(
            ...
            dtype=self._maybe_cast_dtype(get_action_dtype(action_space)),
        )
    ...
    @staticmethod
    def _maybe_cast_dtype(
        dtype: np.typing.DTypeLike | None,
    ) -> np.typing.DTypeLike | None:
        """
...
class MultiOutputRolloutBuffer(MultiOutputBaseBuffer):
    ...
    def reset(self) -> None:
        ...
        self.actions = np.zeros(
            (self.buffer_size, self.n_envs, self.action_dim),
            dtype=get_action_dtype(self.action_space),
        )
        self.rewards = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
...
class MultiOutputDictReplayBuffer(MultiOutputReplayBuffer):
    ...
    def __init__(
        ...
        super(MultiOutputReplayBuffer, self).__init__(
            buffer_size, observation_space, action_space, device, n_envs=n_envs
        )
    ...
    def sample(  # type: ignore[override]
        ...
        :return:
        """
        return super(MultiOutputReplayBuffer, self).sample(
            batch_size=batch_size, env=env
        )
...
class MultiOutputDictRolloutBuffer(MultiOutputRolloutBuffer):
    ...
    def __init__(
        ...
        super(MultiOutputRolloutBuffer, self).__init__(
            buffer_size, observation_space, action_space, device, n_envs=n_envs
        )

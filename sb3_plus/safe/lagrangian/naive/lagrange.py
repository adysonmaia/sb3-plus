...
class Lagrange(BaseLagrange):
    ...
    def __init__(
        ...
        self.optimizer = self.optimizer_class(
            self.multiplier_net.parameters(),
    ...
    def update_multiplier(
        self, mean_ep_cost: float, current_progress_remaining: float, **kwargs
    ) -> th.Tensor:
        """
        ...
        update_learning_rate(
            self.optimizer, self.lr_schedule(current_progress_remaining)
        )
        ...
        th.nn.utils.clip_grad_norm_(
            self.multiplier_net.parameters(), self.max_grad_norm
        )
        self.optimizer.step()
        return loss
...
class LagMultiplier(nn.Module):
    ...
    def __init__(self, init_value: float = 0.0):
        ...
        self.multiplier_parameter = nn.Parameter(
            th.tensor(init_value), requires_grad=True
        )

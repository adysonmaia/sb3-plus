...
def action_flatdim_box(space: spaces.Box) -> int:
    return int(np.prod(space.shape))


@action_flatdim.register
def action_flatdim_discrete(space: spaces.Discrete) -> int:
    return 1  # A data point is an int


@action_flatdim.register
def action_flatdim_multibinary(space: spaces.MultiBinary) -> int:
    return int(space.n)  # Number of binary data points


@action_flatdim.register
def action_flatdim_multidiscrete(space: spaces.MultiDiscrete) -> int:
    return int(len(space.nvec))  # Number of discrete data points


@action_flatdim.register
def action_flatdim_dict(space: spaces.Dict) -> int:
    return int(sum([action_flatdim(s) for s in space.spaces.values()]))


@action_flatdim.register
def action_flatdim_tuple(space: spaces.Tuple) -> int:
...
def action_flatten_box(space, x: UnflattenSpacePoint) -> np.ndarray:
    return np.asarray(x, dtype=space.dtype).flatten()


@action_flatten.register
def action_flatten_dict(space: spaces.Dict, x: UnflattenSpacePoint) -> np.ndarray:
...
@action_flatten.register
def action_flatten_tuple(space: spaces.Tuple, x: UnflattenSpacePoint) -> np.ndarray:
    return np.concatenate(
        [action_flatten(s, x_part) for x_part, s in zip(x, space.spaces)]
    )
...
def action_unflatten_box(space, x: np.ndarray) -> np.ndarray:
    return np.asarray(x, dtype=space.dtype).reshape(space.shape)


@action_unflatten.register
def action_unflatten_discrete(space: spaces.Discrete, x: np.ndarray) -> int:
    return int(x.flat[0])


@action_unflatten.register
def action_unflatten_dict(space: spaces.Dict, x: np.ndarray) -> dict:
...
@action_unflatten.register
def action_unflatten_tuple(space: spaces.Tuple, x: np.ndarray) -> tuple:
